...
const UPGRADES = [
    { id: 'cadencia', name: 'Cadência', desc: 'Tiro +rápido' },
    { id: 'penetracao', name: 'Penetração', desc: 'Atravessa +1 alvo' },
    { id: 'velocidade', name: 'Velocidade', desc: 'Mover +rápido' },
    { id: 'sorte', name: 'Sorte', desc: '+Chance de Drops' },
    { id: 'ima', name: 'Ímã de XP', desc: 'Pega itens mais longe' },
...
const RARITIES = [
    { name: 'Comum', mult: 1, class: 'rarity-comum', color: '#ecf0f1', level: 0 },
    { name: 'Rara', mult: 1.5, class: 'rarity-rara', color: '#3498db', level: 1 },
    { name: 'Épica', mult: 2.2, class: 'rarity-epica', color: '#9b59b6', level: 2 },
    { name: 'Lendária', mult: 3.5, class: 'rarity-lendaria', color: '#f1c40f', level: 3 },
    { name: 'Mítica', mult: 5, class: 'rarity-mitica', color: '#e74c3c', level: 4 },
    { name: 'Secreta', mult: 15, class: 'rarity-secreta', color: '#00ffcc', level: 5 }
];
...
class Drop {
    constructor(x, y, type) {
        this.x = x; this.y = y; this.type = type; this.radius = 10;
...
class Barricade {
    constructor(x, y) { this.x = x; this.y = y; this.radius = 20; this.hp = 100; this.maxHp = 100; }
    draw(ctx) {
        ctx.save(); ctx.translate(this.x - camera.x, this.y - camera.y);
        ctx.fillStyle = '#8B4513'; ctx.fillRect(-20, -10, 40, 20);
        ctx.fillStyle = '#A0522D'; ctx.fillRect(-20, -15, 5, 30); ctx.fillRect(15, -15, 5, 30);
...
class Projectile {
    ...
    draw(ctx) {
        if (this.type === 'Plasma') {
            ...
            ctx.strokeStyle = '#00ffcc'; ctx.lineWidth = 6; ctx.lineCap = 'round';
            ctx.beginPath(); 
...
class Player {
    constructor(x, y) {
        ...
        // Weapons Inventory
        const defaultWeapon = { id: 'w1', name: 'Pistol Comum', type: 'Pistol', rarity: RARITIES[0], damage: 10, fireRateMod: 1 };
...
class Enemy {
    constructor(x, y, type, hpMult = 1) {
        ...
        if (type === 'zombie') { this.radius = 16; this.speed = 1.6; this.hp = 15 * hpMult; }
        else if (type === 'cultist') { this.radius = 18; this.speed = 1.2; this.hp = 30 * hpMult; }
        else if (type === 'brute') { this.radius = 28; this.speed = 0.8; this.hp = 120 * hpMult; }
        else if (type === 'fast') { this.radius = 12; this.speed = 2.8; this.hp = 5 * hpMult; }
    }
...
// --- Main Engine ---
class GameEngine {
    constructor() { this.particleSystem = new ParticleSystem(); this.state = 'MENU'; }
    ...
    reset() {
        ...
        this.level = 1; this.xp = 0; this.maxXp = 10; this.supplies = 0;
            ...
            for(let key in counts) {
                if(counts[key].length >= 3) {
                    ...
                    if (rLevel < RARITIES.length - 2) { 
                        ...
                        const fusedWeapon = {
                            id: Math.random().toString(36).substr(2, 9),
                            name: `${type} ${newRarity.name}`,
                            type: type,
                            rarity: newRarity,
                            damage: 10 * newRarity.mult,
                            fireRateMod: type === 'Minigun' ? 0.3 : (type === 'RPG' ? 2.5 : (type === 'Shotgun' ? 1.5 : (type === 'Plasma' ? 0.1 : 1)))
                        };
    ...
    applyUpgrade(id) {
        ...
        this.level++; this.xp = 0; this.maxXp = Math.floor(this.maxXp * 1.5);
    ...
    fireWeapon(tx, ty) {
        ...
        if (wpn.type === 'Pistol') {
            this.projectiles.push(new Projectile(this.player.x, this.player.y, tx, ty, baseSpd, wpn.damage, this.player.pierce, wpn.type));
        } 
        else if (wpn.type === 'Shotgun') {
            for(let i=-2; i<=2; i++) {
                const angle = Math.atan2(ty - this.player.y, tx - this.player.x) + (i * 0.15);
                this.projectiles.push(new Projectile(this.player.x, this.player.y, this.player.x + Math.cos(angle), this.player.y + Math.sin(angle), baseSpd*0.9, wpn.damage, this.player.pierce, wpn.type));
            }
        }
        else if (wpn.type === 'Minigun') {
            // Slight inaccuracy
            const angle = Math.atan2(ty - this.player.y, tx - this.player.x) + (Math.random() - 0.5)*0.2;
            this.projectiles.push(new Projectile(this.player.x, this.player.y, this.player.x + Math.cos(angle), this.player.y + Math.sin(angle), baseSpd*1.2, wpn.damage, this.player.pierce, wpn.type));
        }
        else if (wpn.type === 'RPG') {
            this.projectiles.push(new Projectile(this.player.x, this.player.y, tx, ty, baseSpd*0.6, wpn.damage, 0, wpn.type));
        }
        else if (wpn.type === 'Plasma') {
            this.projectiles.push(new Projectile(this.player.x, this.player.y, tx, ty, baseSpd*2.5, wpn.damage, this.player.pierce + 100, wpn.type));
        }
        ...
        if (this.player.doubleShotTimer > 0 && wpn.type !== 'Shotgun') {
            ...
            this.projectiles.push(new Projectile(this.player.x, this.player.y, this.player.x + Math.cos(angle), this.player.y + Math.sin(angle), baseSpd, wpn.damage, this.player.pierce, wpn.type));
        }
    ...
    update() {
            ...
            for (let j = this.enemies.length - 1; j >= 0; j--) {
                ...
                if (Math.hypot(p.x - e.x, p.y - e.y) < p.radius + e.radius) {
                    ...
                    if (e.hp <= 0 && this.enemies.includes(e)) {
                        // Drops
                        this.drops.push(new Drop(e.x, e.y, 'xp'));
        ...
        for (let i = this.drops.length - 1; i >= 0; i--) {
            let d = this.drops[i];
            const collectionRadius = this.player.radius + d.radius + this.player.magnetRadius;
            
            // Magnet pull effect
            if (Math.hypot(this.player.x - d.x, this.player.y - d.y) < collectionRadius * 2) {
                const ang = Math.atan2(this.player.y - d.y, this.player.x - d.x);
                d.x += Math.cos(ang) * 5; d.y += Math.sin(ang) * 5;
            }

            if (Math.hypot(this.player.x - d.x, this.player.y - d.y) < this.player.radius + d.radius + 10) {
                if (d.type === 'xp') {
                    this.xp += 2 * this.player.xpMultiplier; if (this.xp >= this.maxXp) this.triggerLevelUp();
